...
Требуется считать данные о продолжительности периода, преобразовать их в тип Duration, а затем вывести (в формате UnixDate) дату и время,
...
func main() {
	...
	var min, sek int64
	fmt.Scanf("%d мин. %d сек.", &min, &sek)
	convertMin := min * 60
	sek = sek + convertMin + now
	res := time.Unix(sek, 0)
