...
func fibonacci(c, quit chan int) { // 	3) запускаем fibonacci
	...
	x, y := 0, 1
	for {
		select { // fibonacci выполняет первый for select:
...
func main() { // 1) запускаем main
	...
	go func() { //2) запускаем анонимную рутину
		...
		quit <- 0 // n+1) fibonacci выполняет очередной for:
	...
	fibonacci(c, quit) //- т.к. в канале "quit" есть значение, то чтение не заблокировано и выполняется второй case, идёт вывод на экран сообщения и завершение функции fibonacci через return
